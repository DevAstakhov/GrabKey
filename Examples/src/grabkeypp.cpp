...
std::unordered_map<
    std::string, std::unordered_map<std::string, std::function<void()>>>
    functions = {
        {"basic",
         {{"reader", use_reader_basic}, {"processor", use_processor_basic}}},
        {"advanced",
...
void print_usage(const char* bin_name) {
    std::cout << "Usage: " << bin_name
              << " <basic | advanced> <reader | processor>" << std::endl;
}
...
void use_processor_basic() {
    ...
    config.handlers[keyboard::Key::ESC] = [&processor](auto) {
        processor.stop();
    };
...
void use_reader_advanced() {
    ...
    public:
        ...
        MyResult process_buffer(std::vector<char> buffer) override {
            ...
            if (buffer[0] == 'w')
                return {MyResult::KEY_W, false};
            if (buffer[0] == 'W')
                return {MyResult::KEY_W, true};
            if (buffer[0] == 's')
                return {MyResult::KEY_S, false};
            if (buffer[0] == 'S')
                return {MyResult::KEY_S, true};
            if (buffer[0] == 'a')
                return {MyResult::KEY_A, false};
            if (buffer[0] == 'A')
                return {MyResult::KEY_A, true};
            if (buffer[0] == 'd')
                return {MyResult::KEY_D, false};
            if (buffer[0] == 'D')
                return {MyResult::KEY_D, true};
            if (buffer[0] == char(27))
        ...
        Result process_timeout() override { return {MyResult::NONE, false}; }
...
void use_processor_advanced() {
    ...
    struct MyResult {
        ...
        operator int() const { return int(key); }
    ...
    config.before_handler = [](MyResult) {
        std::cout << "\r"
                  << "\033[K"
                  << "\r" << std::flush;
    };
    ...
    config.handlers[MyResult::KEY_W] = [](MyResult key) {
        std::cout << "Going UP ";
    };
    config.handlers[MyResult::KEY_S] = [](MyResult key) {
        std::cout << "Going DOWN ";
    };
    config.handlers[MyResult::KEY_A] = [](MyResult key) {
        std::cout << "Turning LEFT ";
    };
    config.handlers[MyResult::KEY_D] = [](MyResult key) {
        std::cout << "Turning RIGHT ";
    };

    // do not forget to add handler which stops the processor
    config.handlers[MyResult::ESC] = [&processor](MyResult) {
        processor.stop();
    };

    // the after_handler adds some common information and flushes the text
    config.after_handler = [](MyResult key) {
        std::cout << (key.key != MyResult::ESC ?
                          (key.is_shift_pressed ? "fast" : "slow") :
                          "")
                  << std::flush;
    };
